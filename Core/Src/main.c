...
volatile uint32_t valor2 = 0;
volatile uint32_t ticks = 0;
volatile uint8_t primera_captura = 0;
volatile float frecuencia_leida = 0;
...
int main(void)
{
  ...
  {
	  char msg[64];
	  int len = sprintf(msg, "Frecuencia: %.2f Hz \r\n", frecuencia_leida);
    ...
    if (htim->Instance == TIM2) {
        if (primera_captura == 0) {
            ...
            if (valor2 > valor1) {
                ...
                ticks = (0xFFFFFFFF - valor1) + valor2;
            }

            if (ticks > 0) {
                frecuencia_leida = 933333.33f / ticks;
            }
            primera_captura = 0;
        }
    }
